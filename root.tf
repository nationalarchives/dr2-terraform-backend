...
module "terraform_github_repository_policy" {
  ...
  policy_string = templatefile("${path.module}/templates/iam_policy/terraform_state_access.json.tpl", { bucket_name = local.terraform_state_bucket_name, dynamo_table_arn = module.terraform_dynamo.table_arn, da_dynamo_table_arn = module.da_terraform_dynamo.table_arn })
}
